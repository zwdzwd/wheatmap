...
The **LeftOf**, **TopOf**, **RightOf** and **Beneath** are for placing new object by anchoring to the edge of existing object.

```{r}
WHeatmap(matrix(1:1)) +
  WHeatmap(matrix(1:2), RightOf()) +
  WHeatmap(matrix(1:3), RightOf()) + 
  WLegendV(NULL, BottomRightOf(h.pad=0.1), label.fontsize = 20, width=0.2, yticklabel.pad=0.05)
```
...
The **TopLeftOf** 
```{r}
WHeatmap(matrix(1:9,nrow=3)) + 
  WHeatmap(matrix(1:16,nrow=4), BottomRightOf(just='topleft')) + 
  WHeatmap(matrix(1:9,nrow=3), BottomLeftOf(just='topright')) + 
  WHeatmap(matrix(1:16,nrow=4), TopLeftOf(just='bottomright'))
```
